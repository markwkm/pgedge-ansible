...
| Parameter | Use Case |
|-----------|----------|
| `lakekeeper_host` | Address Lakekeeper binds and is reached at. Shared with `setup_coldfront`, which needs to know where the catalog is. |
| `lakekeeper_bind_ip` | Address the Lakekeeper service binds. Defaults to `lakekeeper_host`. |
