...
- Type: Boolean
- Default: `false`
- Description: When true, adds `install_coldfront` and `setup_coldfront`
  to a pgEdge node. Set this in the `pgedge` group's inventory variables,
  alongside the Lakekeeper and object store settings below.

```yaml
pgedge:
  vars:
    coldfront_enabled: true
```
